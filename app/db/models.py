...
class User(Base):
    ...
    # Max number of simultaneous IPs allowed to use subscription (None = no limit).
...
class UserSubscriptionIP(Base):
    """Tracks IPs that recently fetched subscription (for ip_limit enforcement)."""
    __tablename__ = "user_subscription_ips"
    __table_args__ = (UniqueConstraint("user_id", "ip", name="uq_user_subscription_ip_user_ip"),)
