      ...
      .catch((err) => {
        ...
        if (err?.response?.status === 422) {
          Object.keys(err.response._data.detail).forEach((key) => {
            ...
            form.setError(
              key as "proxies" | "username" | "data_limit" | "expire" | "ip_limit",
              {
                type: "custom",
                        ...
                        <Controller
                          ...
                          render={({ field }) => (
                            <Input
                              ...
                              disabled={disabled}
                              error={form.formState.errors.ip_limit?.message as string}
                              value={field.value != null ? String(field.value) : ""}
                              placeholder={t("userDialog.ipLimitPlaceholder")}
                            />
                          )}
                        />
                        <FormHelperText>{t("userDialog.ipLimitHelp")}</FormHelperText>
                      </FormControl>
