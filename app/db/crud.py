...
def create_user(db: Session, user: UserCreate, admin: Admin = None) -> User:
    ...
    dbuser = User(
        ...
        ip_limit=user.ip_limit if getattr(user, "ip_limit", None) else None,
        next_plan=NextPlan(
...
def update_user(db: Session, dbuser: User, modify: UserModify) -> User:
    ...
    if modify.ip_limit is not None:
        dbuser.ip_limit = modify.ip_limit if modify.ip_limit else None
...
def check_subscription_ip_limit(
    db: Session, user_id: int, ip_limit: Optional[int], client_ip: Optional[str]
) -> bool:
    """
    Returns True if the client_ip is allowed to fetch subscription (under ip_limit).
    Returns False if ip_limit is exceeded for this user (too many different IPs recently).
    If ip_limit is None or 0, always returns True.
    """
    if not client_ip or not ip_limit or ip_limit <= 0:
        return True
    cutoff = datetime.utcnow() - timedelta(minutes=SUB_IP_LIMIT_WINDOW_MINUTES)
    db.execute(
        delete(UserSubscriptionIP).where(
            and_(UserSubscriptionIP.user_id == user_id, UserSubscriptionIP.last_seen < cutoff)
        )
    )
    db.commit()
    existing = (
        db.query(UserSubscriptionIP)
        .where(UserSubscriptionIP.user_id == user_id)
        .all()
    )
    ips = {row.ip for row in existing}
    if client_ip in ips:
        return True
    if len(ips) >= ip_limit:
        return False
...
def record_subscription_ip(db: Session, user_id: int, client_ip: str) -> None:
    ...
    row = (
        ...
        .where(
            and_(
                UserSubscriptionIP.user_id == user_id,
                UserSubscriptionIP.ip == client_ip,
            )
        )
        .first()
    )
    ...
    if row:
        ...
        db.add(UserSubscriptionIP(user_id=user_id, ip=client_ip, last_seen=now))
    db.commit()
