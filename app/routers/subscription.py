...
def user_subscription(
    ...
    if not crud.check_subscription_ip_limit(db, dbuser.id, getattr(dbuser, "ip_limit", None), client_ip):
        raise HTTPException(
            status_code=403,
            detail="IP limit exceeded. Too many different devices/locations using this subscription.",
        )
    crud.record_subscription_ip(db, dbuser.id, client_ip)
