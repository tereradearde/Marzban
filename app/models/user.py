...
class User(BaseModel):
    ...
    ip_limit: Optional[int] = Field(
        None, ge=0, nullable=True,
        description="Max simultaneous IPs allowed for subscription (0 or None = no limit)"
    )

    next_plan: Optional[NextPlanModel] = Field(None, nullable=True)

    @field_validator('data_limit', 'ip_limit', mode='before')
    def cast_to_int(cls, v):
        ...
        if isinstance(v, int):  # Allow integers directly
            return v
        raise ValueError("data_limit/ip_limit must be an integer or a float, not a string")  # Reject strings
